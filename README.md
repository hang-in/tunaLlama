...
tunaLlama 는 프롬프트 시드나 AGENTS.md 템플릿이 아닙니다. 상위 모델 세션이
모든 문서와 긴 코드를 직접 끌어안는 대신, **긴 코드 생성을 로컬/저비용 LLM 에
위임하고 Architect 는 분해와 검증에 집중하도록 만드는 MCP 기반 delegation
runtime** 입니다.

**상태**: **v0.5.x usable dogfooding release** (2026-05-11). Claude Code +
Codex CLI 양쪽에서 MCP tool 호출은 검증됐지만, organic dogfooding 측정과
외부 사용자 재현성은 계속 수집 중입니다.
...
실제 한도 절약 효과는 사용자의 작업 유형, 모델, provider latency, Architect
의 검증 방식에 따라 달라집니다. Anthropic / OpenAI 의 정액제 한도 계산식이
공개되어 있지 않으므로, tunaLlama 는 "정량 절감 보장" 이 아니라 **긴 생성
작업을 위임해 상위 모델 사용량을 줄일 수 있는 구조** 를 제공합니다. 실 가치
는 본인 dogfooding 으로 확인 권장.
...
3. 백엔드가 generate → review → fix 루프를 반복 (bounded delegation - 종료
   조건은 review pass 또는 max iter). 모든 호출은 SQLite 에 기록되고 한국어
   형태소로 색인됩니다.
4. 검색의 진짜 가치: **mid-size 로컬 LLM 의 컨텍스트 한계를 architect 가
   보완** (Opus + Sonnet subagent 패턴과 동일). Phase 7-2 **합성 시드 기반
   측정** 에서 context boost +0.58 ~ +0.64 관측. 실사용 효과 확정 수치는
   아니며, organic dogfooding metric 은 별도 수집 중입니다.
...
설치 후 Claude Code / Codex 세션에서. Architect 가 자동 호출할 수 있지만,
첫 사용과 재현 가능한 워크플로우에서는 **명시 호출 경로를 권장** 합니다.
...
**v0.5.x usable dogfooding release**. Claude Code + Codex CLI 양쪽에서
MCP tool 호출 검증, 단 organic dogfooding (실 일상 사용) 측정 자산은 수집
중입니다. 아래 한계는 카테고리별 정리:

### 1. 사용 한도 / 비용 관련

- **사용 한도 절약은 체감 데이터**. Anthropic / OpenAI 정액제 한도 계산식이
  비공개라 정량 측정 불가능.
- **MCP 도구 system prompt 비용은 의도된 trade-off** 입니다. 13 도구
  description + schema 가 매 conversation 의 system prompt 에 prepend 되며
  추정치는 약 1.6k tokens. 이는 accidental context bloat 가 아니라,
  Architect 가 적절한 delegation 도구를 선택하기 위한 affordance 비용입니다.
  tunaLlama 는 이 비용을 제거하기보다 도구 수와 description 품질을 관리해
  호출 적중률과 검증 품질을 유지하는 쪽을 선택합니다. 자세한 측정:
...
- **organic dogfooding metric 수집** (v0.5.7+). 매 delegation 후 4 종
...
- **Architect 호출 의존**. `tuna_*` 도구는 Architect 가 작업 컨텍스트 보고
  호출하는 구조 - 도구 description 품질이 호출 적중률을 결정합니다. 첫 사용
  과 재현 가능한 워크플로우에서는 **명시 호출 권장**.
- **Subagent 자동 인식 미작동** (Codex 0.128.0 실측): `plugin/agents/
  ...
  레벨에서 가능.
- **MCP resource auto-attach + SessionStart hook 미작동** (Codex):
  `tunallama://memory/state` resource 는 양 클라이언트 모두 세션 시작 시
  attach 안 됨. v0.5.2 의 SessionStart hook (`plugin/hooks/session_start.py`)
  은 Claude Code v0.5.5+ 에서만 작동 - Codex 는 미인식. **실제 권장 운영**:
  Claude Code 는 hook 으로 state.md auto-prepend, Codex 는 사용자가
  `tuna_load_memory` 명시 호출.

### 4. 로컬 LLM / provider

- **로컬 LLM 의존**. Ollama / LM Studio / Ollama Cloud 중 하나 없으면 작동 X.
...
tunaLlama 는 에이전트에게 더 많은 문서를 읽히는 방식으로 컨텍스트 한계를
해결하려 하지 않습니다. 대신 작업 단위를 작게 잘라 MCP 도구로 로컬/저비용
LLM 에 넘기고, 상위 Architect 모델은 짧은 spec, review 결과, 최종 diff 판단
에 집중합니다.

문서 기반 운영 규칙은 시간이 지나면 stale state, drift, lost-in-the-middle
문제를 만들 수 있습니다. tunaLlama 는 이를 피하기 위해 delegation call 을
SQLite 에 기록하고, 필요할 때 검색 / 리콜하는 실행 계층을 둡니다.
